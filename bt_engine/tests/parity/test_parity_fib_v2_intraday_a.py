"""BLOCKING parity gate: FibV2IntradayA bt_engine output vs research dedup parquet.

Compares bt_engine port over full 21yr XAU OANDA to
research/fib_retrace/intraday_dedup/intraday_a_trades.parquet (generated by
gen_intraday_dedup_parquets.py).
...
RESEARCH_PARQUET = Path(
    ...
    "intraday_a_trades.parquet"
)
MAX_BARS_HELD_A = 48  # 12h * 4 bars/h — STRICT cap to match research (no doubling)
...
def test_parity_intraday_a_count_and_pnl():
    ...
    print(f"\n[parity A] bt n={n_bt}  research n={n_research}  diff={count_diff_pct:.2f}%")
    print(f"[parity A] bt net_r={b_net:+.1f}  research net_r={r_net:+.1f}  diff={net_diff_pct:.2f}%")
    print(f"[parity A] bt PF={b_pf:.3f}  research PF={r_pf:.3f}  diff={pf_diff_pct:.2f}%")
