...
def _save_to_db(req, stats, trades, equity_curve, duration_ms):
    ...
    run_id = insert_returning(
        """INSERT INTO gd_backtest_runs
           ...
           RETURNING id""",
        (["alpha_sweep_oil"], req.start_date, req.end_date, float(req.capital), float(req.risk_pct),
        ...
        with conn.cursor() as cur:
            for i, t in enumerate(trades):
                cur.execute(
                    ...
                    (run_id, i, str(t["date"]), int(t["year"]), int(t["month"]), "alpha_sweep_oil", t["direction"],
