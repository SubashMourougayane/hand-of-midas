"""Pessimistic critic postmortem of 2009 Oil Micro backtest.

Premise: dashboard showed >1000% / 4000%+ years on Oil Micro. 2009 specifically
during the GFC oil crash → recovery. That's a regime where a fade strategy
either gets killed or hits the jackpot. Critic mode: assume the result is
artifact until proven otherwise.

Steps:
  1. Run real run_backtest() on the actual 21yr CSVs.
  2. Isolate 2009 Oil Micro trades.
  ...
  6. Write docs/POSTMORTEM_2009_OIL_MICRO.md + scripts/output/postmortem_2009.html
...
def main():
    print("=" * 80)
    print("PESSIMISTIC POSTMORTEM — 2009 Oil Micro")
    print("=" * 80)
    ...
    print("[3/6] Isolating 2009 trades...")
    trades_2009 = [t for t in all_trades if t.year == 2009]
    print(f"      2009 trades: {len(trades_2009)}")

    if len(trades_2009) == 0:
        print("      No 2009 trades — aborting")
        return

    # Convert to dicts
    td = [trade_to_dict(t) for t in trades_2009]
    ...
    random.seed(2009)  # deterministic
    sampled = []
    for (y, m), data in sorted(monthly.items()):
        if data["n"] == 0:
            continue
    ...
    # Save MD report
    md_path = os.path.join(DOCS_DIR, "POSTMORTEM_2009_OIL_MICRO.md")
    ...
    # Save HTML report
    html_path = os.path.join(OUT_DIR, "postmortem_2009.html")
    ...
    # Save raw data
    raw_path = os.path.join(OUT_DIR, "postmortem_2009_raw.json")
...
def write_md_report(f, trades, monthly, flags, samples,
    ...
    f.write(f"""# Postmortem — 2009 Oil Micro
...
Random sample, seed=2009, of 4 trades per month — {len(samples)} total. For each: trade card + surrounding H1 OHLC + plausibility check (was entry price within the H1 bar's range?).
...
_Generated by `scripts/postmortem_2009_oil_micro.py` on {datetime.now(timezone.utc).isoformat()}._
""")
...
<head>
<meta charset="utf-8">
<title>2009 Oil Micro — Pessimistic Postmortem</title>
<style>
...
<body>
<h1>2009 Oil Micro — Pessimistic Postmortem</h1>
