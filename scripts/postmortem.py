...
def render_markdown(trade: dict, journal: list, m3_window: list, peers: list,
                    ...
                    system_label: str, instrument: str,
                    bt_replay_result: dict | None = None) -> str:
    ...
    # ── BT Parity Replay ────────────────────────────────────────────────────
    # Run the system's BT for this trade's signal date and compare what BT
    # *would have* produced vs what live actually produced. Forward-only
    # measurement of live↔BT parity per trade.
    if bt_replay_result is not None:
        try:
            from bt_replay import render_replay_section
            lines.extend(render_replay_section(bt_replay_result, instrument))
        except Exception as e:
            lines.append("## BT Parity Replay")
            lines.append("")
            lines.append(f"- ⚠️ Replay rendering failed: `{type(e).__name__}: {e}`")
            lines.append("")
...
def gather_and_write(trade_ref: str) -> str:
    ...
    # BT Parity Replay — forward-only measurement of live↔BT parity per trade.
    # For Filter #27 trades, prefer LIMIT_FILLED time; for market entries use
    # ENTRY_FILLED. Falls back to trade.entry_time. Failures here NEVER abort
    # the postmortem — they degrade to a "replay unavailable" section.
    bt_replay_result = None
    try:
        from bt_replay import replay as _bt_replay
        # Prefer LIMIT_FILLED context (broker fill time) for F27 trades.
        # Otherwise fall back to ENTRY_FILLED, finally to trade.entry_time.
        replay_anchor_iso = None
        for e in journal:
            if e.get("event_type") == "LIMIT_FILLED":
                replay_anchor_iso = e.get("timestamp")
                break
        if not replay_anchor_iso:
            for e in journal:
                if e.get("event_type") == "ENTRY_FILLED":
                    replay_anchor_iso = e.get("timestamp")
                    break
        if not replay_anchor_iso:
            replay_anchor_iso = trade.get("entry_time")
        if replay_anchor_iso:
            bt_replay_result = _bt_replay(
                trade_ref,
                replay_anchor_iso,
                trade.get("side", "LONG"),
                verbose=False,
        ...
        bt_replay_result = {
            ...
            "match": None,
            "candidates_nearby": [],
            "drift_reason": f"replay raised: {type(e).__name__}: {e}",
        }
    ...
    md = render_markdown(trade, journal, bars_window, peers, excursions,
                         ...
                         bt_replay_result=bt_replay_result)
