"""Filter #16 — R:R lower bound (0.8 → 1.5) pre/post backtest.

Audit-discovered: current floor of 0.8 allows trades needing ~80% WR to
break even — those are the statistically-bad slice. Filter #16 raises
to 1.5 (TP must be at least 1.5× the risk distance).

Implementation: rr_lower_bound kwarg threaded through all 4
generate_signals() functions. Read from per-system ALPHA_SWEEP /
MICRO_ALPHA_SWEEP config via .get(default 0.8). Live scheduler reads
the same key (drift bug #6 prevention).

For Filter #16 (signal-gate), this changes WHICH signals fire, so
cross-checking BT vs Live signal-gen via parity harness is non-negotiable.
"""
...
# Filter #16 parameters
BASELINE_RR = 0.8   # current production
FILTER_RR = 1.5     # tested floor
...
def main():
    print("=" * 70)
    print("FILTER #16 — R:R lower bound (0.8 → 1.5)  (Pre/Post Backtest)")
    print("=" * 70)
    print(f"Baseline: rr_lower_bound = {BASELINE_RR} (current production)")
    print(f"Filter:   rr_lower_bound = {FILTER_RR}")
    print("Hypothesis: Sub-1.5R trades need ~80% WR to break even.")
    print("            Removing them should improve PF by skipping the worst geometry.")
    print("=" * 70)

    results = {"filter": 16, "name": "R:R lower bound 1.5",
               "generated": datetime.now(timezone.utc).isoformat(),
               "params": {"baseline_rr": BASELINE_RR, "filter_rr": FILTER_RR},
               "baseline": {}, "filter_a": {}}

    for s in SYSTEMS:
        print(f"\n[{s['label']}]")
        print(f"  baseline (rr={BASELINE_RR})...")
        b = run_one(s["label"], s["pkg"], BASELINE_RR)
        print(f"    {fmt(b)}  ({b['elapsed_s']:.1f}s)")
        results["baseline"][s["key"]] = b

        print(f"  filter   (rr={FILTER_RR})...")
        f = run_one(s["label"], s["pkg"], FILTER_RR)
        print(f"    {fmt(f)}  ({f['elapsed_s']:.1f}s)")
        results["filter_a"][s["key"]] = f

    json_path = os.path.join(OUT_DIR, "filter_16_results.json")
    with open(json_path, "w") as fh:
        json.dump(results, fh, indent=2, default=str)
    print(f"\n✓ wrote {json_path}")

    # Telegram summary
    lines = ["🧪 <b>FILTER #16 — R:R lower bound 1.5</b>", "",
             f"Skip signals where (tp − entry) / risk < {FILTER_RR}", "",
             "BT 21-yr (all 4 systems, baseline = current production 0.8):"]
    total_b = 0
    total_f = 0
    for s in SYSTEMS:
        ...
        total_b += b["pnl"]
        total_f += f["pnl"]
        # N reduction is expected — that's the point
    ...
    lines.append("")
    lines.append("Reply: ship 16 [systems] / stash 16 / next")
