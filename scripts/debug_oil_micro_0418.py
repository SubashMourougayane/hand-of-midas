"""Trace Oil Micro BT for 2026-06-17 04:18 SHORT trade. Same patch pattern
as scripts/debug_oil_15_09_short_v2.py.
"""
...
def patched_execute(df, bar_start, entry, sl, tp, direction, max_bars, strategy, **kwargs):
    ...
    target_ts = pd.Timestamp("2026-06-17 04:18:00", tz="UTC")
    if bar_ts == target_ts and direction == "short":
        ...
        for i in range(15):
            ...
            print(f"   [{i}] {ts}: ask o={ao:.4f} h={ah:.4f} l={al:.4f} c={ac:.4f}")
...
print("Running Oil Micro BT with patched execute_trade...")
