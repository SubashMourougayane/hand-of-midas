"""Micro price stream — XAU_USD tick-by-tick for break-even detection (MT5 mode: disabled)."""
import json
import threading
import time
        ...
        try:
            with httpx.stream("GET", url, headers=HEADERS, timeout=None) as resp:
                if resp.status_code != 200:
                    _log_journal("SYSTEM", "micro_alpha_sweep", "STREAM_ERROR", None, {"status": resp.status_code})
                    time.sleep(5)
                    continue
                print(f"  [MICRO STREAM] Connected — receiving XAU_USD ticks")
                _log_journal("SYSTEM", "micro_alpha_sweep", "STREAM_CONNECTED", None, {"instrument": "XAU_USD"})
            ...
            if _running:
                print(f"  [MICRO STREAM] Disconnected: {e}. Reconnecting...")
                _log_journal("SYSTEM", "micro_alpha_sweep", "STREAM_DISCONNECTED", None, {"error": str(e)})
                time.sleep(3)
...
def start_stream():
    global _running
    if EXECUTOR == "mt5":
        print("MT5 mode — OANDA price stream disabled for Micro (DWX handles prices)")
        return
    if _running:
        return
    _running = True
    t = threading.Thread(target=_stream_loop, daemon=True, name="micro-price-stream")
    t.start()
    print("Micro price stream started (XAU_USD tick-by-tick)")
