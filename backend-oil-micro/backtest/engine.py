...
def generate_signals(oil_h1: pd.DataFrame, oil_m3: pd.DataFrame, daily_bias: dict,
                     ...
                     anti_trend_lookback_h: int = 4) -> list[Signal]:
...
def run_backtest(
    ...
    anti_trend_threshold: Filter #4 — skip if 4hr H1 move > threshold * ATR_14.
    """
    ...
    if anti_trend_threshold is None:
        anti_trend_threshold = MICRO_ALPHA_SWEEP.get("anti_trend_threshold", 0.0)
    np.random.seed(seed)
