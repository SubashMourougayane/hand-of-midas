...
def _compute_tick_age_secs(tick_time_str: str) -> Optional[float]:
    ...
    string ("2026.06.18 15:18:14") — server is UTC+2/UTC+3 depending on
    DST. We can't trust the absolute UTC offset across runs, so we
    compute the LOCAL-clock-side delta: parse the tick string as if it
    were in the SAME timezone as our process clock, and subtract from
    `datetime.now()`. The clock-side delta is what matters for
    "how-stale" — if DWX wrote 90s ago by its own clock, our clock
    will see ~90s gap regardless of timezone offset.
    """
    ...
    try:
        ...
        return None
    # Both naive — same clock-side semantics
...
def execute_signal(strategy: str, direction: str, entry_price: float, sl_price: float, tp_price: float,
    ...
    # Filter #27: limit-order entry. Mirror of backend-oil canonical implementation.
    ...
    if cfg_entry_mode == "limit":
        ...
        # OIL_MICRO_LIMIT_DRY_RUN wins, falls back to global LIMIT_DRY_RUN.
        dry_run = parse_dry_run_env(system_prefix="OIL_MICRO")
        ...
        if intended_limit is not None:
            ...
            if not dry_run:
                ...
                print(f"  [OIL-MICRO] Placing {direction.upper()} {units} barrels @ LIMIT "
                ...
                if not limit_result.get("success"):
                    ...
                    _log_journal(trade_ref, strategy, "LIMIT_ORDER_FAILED", entry_price,
                                 {"error": error, "instrument": "BCO_USD",
                                  "intended_limit": intended_limit})
                    print(f"  [OIL-MICRO] Limit order FAILED: {error}")
                    return None
                ...
                try:
                    ...
                    print(f"  [OIL-MICRO] DB INSERT FAILED (limit is pending on broker!): {e}")
                    try:
                ...
                try:
                    ...
                    print(f"  [OIL-MICRO] notify.limit_placed swallowed exception: {e}")
                ...
                try:
                    ...
                    print(f"  [OIL-MICRO] _log_signal FAILED (limit path): {e}")
                print(f"  [OIL-MICRO] LIMIT PLACED: ticket={pending_ticket} @ ${intended_limit:.4f}")
                return trade_ref
...
def check_open_positions():
    ...
    # Filter #27: exclude pending limits — see backend-oil/scanner/live_engine.py canonical comment.
...
def check_alpha_sweep_breakeven():
    """Check break-even for Oil Micro trades. Filter #27: exclude pending limits."""
    open_trades = execute(
        f"SELECT * FROM gd_trades WHERE exit_time IS NULL AND oanda_trade_id IS NOT NULL "
        f"AND trade_ref LIKE '{TRADE_REF_PREFIX}%%' AND COALESCE(mode, 'live') != 'pending'",
        fetch=True
    ...
    # phantom BE arms caused by stale market_data.json. See
    # docs/30-day-challenge/ideas/IDEAS.md "PHANTOM BE arm" entry.
...
# Mirror of backend-oil canonical implementation. See that file for state machine docs.
...
def _orphan_lookup_ttl_seconds(trade_ref: str) -> Optional[int]:
    ...
    # Path 2: config fallback (Oil Micro uses MICRO_ALPHA_SWEEP).
    try:
...
def reconcile_broker_pending_orphans():
    ...
    for ticket_str, info in pending_file.items():
        ...
        synthetic_ref = f"OIL-MI-orphan-pend-{ticket[-8:]}"
        _log_journal_safe(synthetic_ref, "micro_alpha_sweep_oil",
...
def pending_order_monitor():
    """Filter #27 periodic reconciler. See backend-oil canonical implementation.
    ...
    pending_db = execute(
        ...
        f"AND strategy='micro_alpha_sweep_oil' AND COALESCE(mode, 'live') = 'pending' "
        f"AND trade_ref LIKE '{TRADE_REF_PREFIX}%%'",
        fetch=True
    )
    if not pending_db:
        ...
        if ticket in open_tickets:
            ...
            try:
                ...
                _log.exception("BROKER", "limit_filled_notify_failed",
                               trade_ref=trade_ref, err=str(e))
            _orphan_alerted.discard(ticket)
            print(f"  [OIL-MICRO] LIMIT FILLED: {trade_ref} ticket={ticket} @ ${actual_fill:.4f}")
            continue
        ...
        if ticket in cancelled_tickets:
            ...
            try:
                ...
                _log.exception("BROKER", "limit_ttl_notify_failed",
                               trade_ref=trade_ref, err=str(e))
            _orphan_alerted.discard(ticket)
            print(f"  [OIL-MICRO] LIMIT EXPIRED: {trade_ref} ticket={ticket} @ ${intended_limit:.4f}")
            continue
        ...
        if past_grace:
            ...
            try:
                ...
                _log.exception("BROKER", "limit_ttl_grace_notify_failed",
                               trade_ref=trade_ref, err=str(e))
            _orphan_alerted.discard(ticket)
            print(f"  [OIL-MICRO] LIMIT EXPIRED (grace fallback): {trade_ref} ticket={ticket} @ ${intended_limit:.4f}")
            continue
