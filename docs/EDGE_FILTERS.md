...
EDGE_FILTERS is the umbrella name for **7 ideas that filter or refine these low-edge setups before they enter live.** Three are signal-generation filters (skip the trade), one is a TP-feasibility check, three are execution refinements (change how the trade is managed once entered).
