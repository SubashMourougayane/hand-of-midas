"""Cross-symbol test on USD/JPY (Dukascopy) — Fib V2 A+D ensemble.
...
= 50 pips). $/lot for a given risk = risk_units(JPY) * 100000 / USDJPY_rate.
"""
...
def load_jpy():
    h1 = pd.read_parquet("/tmp/oanda_jpy_h1.parquet")
    m5 = pd.read_parquet("/tmp/oanda_jpy_m5.parquet")
    for df in (h1, m5):
...
def run_fib_ensemble_jpy():
    print("[JPY] loading H1 + M5...")
    h1, m5 = load_jpy()
    ...
    print("\n=== FIB V2 ENSEMBLE on USD/JPY (A + D) ===\n")
    ...
    lt = simulate_fixed_tp(m5_f, long_sigs, horizon_bars=72*12*2, cost_usd=JPY_COST)
    st = simulate_fixed_tp(m5_f, short_sigs, horizon_bars=72*12*2, cost_usd=JPY_COST)
    ...
    for d in (1, 5, 10):
        ...
        ens_d = pd.concat([
            simulate_fixed_tp(m5_f, sl, horizon_bars=72*12*2, cost_usd=JPY_COST),
            simulate_fixed_tp(m5_f, ss, horizon_bars=72*12*2, cost_usd=JPY_COST),
        ], ignore_index=True)
        print_headline(f"  +{d} bar delay", headline(ens_d))
    for extra in (0.010, 0.020):
        ens_c = pd.concat([
            simulate_fixed_tp(m5_f, long_sigs, horizon_bars=72*12*2, cost_usd=JPY_COST + extra),
            simulate_fixed_tp(m5_f, short_sigs, horizon_bars=72*12*2, cost_usd=JPY_COST + extra),
        ], ignore_index=True)
    ...
    ens.to_parquet("/Users/subash/SUBASH/GoldDigger/research/cross_symbol/jpy_fib_ensemble_trades.parquet")

    # ── $1,000 · 3% risk · monthly reset PnL ──
    print("\n=== $1,000 · 3% risk · MONTHLY RESET PnL — USD/JPY A+D ===")
    ...
    for r in df.itertuples(index=False):
        ...
        if ru <= 0: continue
        rate = float(r.entry_price)                  # USDJPY
        dollar_per_lot = ru * 100000 / rate          # $ risk per 1.0 std lot
        if dollar_per_lot <= 0: continue
        raw_lots = (nav * 0.03) / dollar_per_lot
        ...
        nav += float(r.net_r) * lots * dollar_per_lot
