"""Cross-symbol test on GBP/USD OANDA data — Fib V2 ensemble + Martin Luke.
SAME rules + params as XAU. Cost = $0.00005 (0.5 pip = typical GBP/USD spread).
"""
...
GBP_COST = 0.00005  # 0.5 pip spread


def load_gbp():
    h1 = pd.read_parquet("/tmp/oanda_gbp_h1.parquet")
    m5 = pd.read_parquet("/tmp/oanda_gbp_m5.parquet")
...
def run_fib_ensemble_gbp():
    print("[GBP] loading H1 + M5...")
    h1, m5 = load_gbp()
    ...
    lt = simulate_fixed_tp(m5_f, long_sigs, horizon_bars=72*12*2, cost_usd=GBP_COST)
    st = simulate_fixed_tp(m5_f, short_sigs, horizon_bars=72*12*2, cost_usd=GBP_COST)
    ...
    for d in [1, 5, 10]:
        ...
        ens_d = pd.concat([
            simulate_fixed_tp(m5_f, sl, horizon_bars=72*12*2, cost_usd=GBP_COST),
            simulate_fixed_tp(m5_f, ss, horizon_bars=72*12*2, cost_usd=GBP_COST),
        ], ignore_index=True)
        print_headline(f"+{d} delay", headline(ens_d))
    for extra in [0.00010, 0.00020]:
        ens_c = pd.concat([
            simulate_fixed_tp(m5_f, long_sigs, horizon_bars=72*12*2, cost_usd=GBP_COST + extra),
            simulate_fixed_tp(m5_f, short_sigs, horizon_bars=72*12*2, cost_usd=GBP_COST + extra),
        ], ignore_index=True)
        print_headline(f"cost+${extra:.5f}", headline(ens_c))

    out_dir = Path("/Users/subash/SUBASH/GoldDigger/research/cross_symbol")
    ens.to_parquet(out_dir / "gbp_fib_ensemble_trades.parquet")

    # $5k 3% PnL (1 lot = 100k notional, $1/pip standard)
    PIP_VALUE_PER_LOT = 10  # $10 per 0.0001 pip per 1 standard lot (100k)
    print("\n=== $5k 3% monthly reset PnL — GBP ENSEMBLE ===")
...
def run_martin_luke_gbp():
    print("\n\n" + "=" * 80)
    print(">>> MARTIN LUKE on GBP/USD <<<")
    _, m5 = load_gbp()
    m5_f = add_m5_features(m5)
    ...
    for i in sig_idx:
        ...
        trades.append({
            ...
            "cost_r": GBP_COST / risk,
            "net_r": float(outcome_r) - GBP_COST / risk,
    ...
    print_headline("MARTIN LUKE GBP", headline(tdf))
    print("\n[Per-year]")
    ...
    out_dir = Path("/Users/subash/SUBASH/GoldDigger/research/cross_symbol")
    tdf.to_parquet(out_dir / "gbp_martin_luke_trades.parquet")
    return tdf


def main():
    fib = run_fib_ensemble_gbp()
    ml = run_martin_luke_gbp()
    print("\n\n=== GBP CROSS-SYMBOL SUMMARY ===")
    print(f"  Fib V2 Ensemble GBP: {len(fib) if fib is not None else 0} trades")
    if ml is not None:
        print(f"  Martin Luke GBP:     {len(ml)} trades")
