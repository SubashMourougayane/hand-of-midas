...
def _run_alpha_sweep_core(now: datetime, h1_candles: list, daily_candles: list,
    ...
    if not dry_run:
        ...
        existing = execute(
            ...
            "WHERE strategy='alpha_sweep' AND entry_time::date = %s "
    ...
    for sweep_dir, sweep_wick, sweep_ts in sweeps:
        ...
        # Issue #9 fix 2026-06-15: was in-memory only — restart wiped it.
        # Now: in-memory hot cache + DB-backed (gd_traded_sweeps) for restart safety.
        sweep_key = f"{sweep_ts}_{sweep_dir}"
        if sweep_key not in _traded_sweeps_macro["keys"] and not dry_run:
            from backend.db import is_sweep_consumed
            if is_sweep_consumed("gold-macro", today, sweep_key):
        ...
        for j in range(2, len(relevant_m3)):
            ...
            _traded_sweeps_macro["keys"].add(sweep_key)
            if not dry_run:
                # Issue #9 fix: persist for restart safety.
                from backend.db import mark_sweep_consumed
                try:
                    mark_sweep_consumed("gold-macro", today, sweep_key)
                except Exception as e:
