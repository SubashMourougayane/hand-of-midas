...
def execute_signal(strategy: str, direction: str, entry_price: float, sl_price: float, tp_price: float, context: dict = None):
    ...
    try:
        ...
        _log.exception("DB", "trade_insert_failed_orphan_risk", trade_ref=trade_ref, oanda_id=oanda_trade_id, fill=fill_price, strategy=strategy, err=str(e))
        ...
        if not details:
            ...
            if streak == _EXIT_AMBIGUOUS_ALERT_CYCLES:
                try:
                    ...
                    # Issue #14 fix 2026-06-15: was bare pass — alert lost forever.
                    # Now: log so we know notify failed; reset streak by 1 so the
                    # next cycle re-tries the alert.
        ...
        if side == "LONG":
            ...
            if price["bid"] >= target_50:
                ...
                if result.get("success"):
                    ...
                    _log_journal(trade["trade_ref"], "alpha_sweep", "BREAK_EVEN_FAILED", None, {
                        "old_sl": sl, "attempted_sl": new_sl, "error": result.get("error", "Unknown"), "source": "scheduler",
                        "tick_time": tick_time_str, "tick_age_secs": tick_age_secs,
                    })
        else:  # SHORT
            if tp >= entry:
...
def check_alpha_sweep_partial_tp():
    """Filter #7 — bank `partial_tp_size` of position when bar reaches halfway.
    Mirror of backend-oil/scanner/live_engine.py:check_alpha_sweep_partial_tp.
    Strategy='alpha_sweep', instrument='XAU_USD', config from backend.config.ALPHA_SWEEP.
    """
    ...
    open_trades = execute(
        "SELECT * FROM gd_trades WHERE exit_time IS NULL AND strategy='alpha_sweep' "
    ...
    for trade in open_trades:
        try:
            ...
            if not result.get("success"):
                ...
                _log_journal_safe(trade["trade_ref"], "alpha_sweep", "PARTIAL_TP_FAILED", None, {
                    "units_to_close": units_to_close,
                    "error": result.get("error"), "retcode": result.get("retcode"),
                })
                continue
            ...
            _log_journal_safe(trade["trade_ref"], "alpha_sweep", "PARTIAL_TP", fill_price, {
            ...
            try:
                ...
                print(f"  [GOLD] notify.partial_tp swallowed exception: {e}")

            print(f"  [alpha_sweep] PARTIAL_TP {trade['trade_ref']} {side}: closed "
